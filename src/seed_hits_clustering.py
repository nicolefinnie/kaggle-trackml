...
#FEATURE_MATRIX_6 = ['sin', 'cos', 'zarc']

SCALED_DISTANCE_7 = [1,       1,       0.5,  0.00175, 0.00175] 
FEATURE_MATRIX_7 = ['sin', 'cos', 'zarc2', 'px', 'py']
#SCALED_DISTANCE_7 = [1,       1,       0.5]
#FEATURE_MATRIX_7 = ['sin', 'cos', 'zarc2']
            ...
            if os.path.exists(label_file):
                ...
                if self.model_parameters[0] is HELIX_UNROLL_R0_MODE:
                    ...
                    for ii, r0 in enumerate(tqdm(r0_list)):
                        ...
                        ##dfh['zarc'] = dfh.zshift/ (np.arcsin(dfh.cos_theta) * 2 * r0 )
    ...
    for i in range(len(labels_subset)):
        ...
        if os.path.exists(label_file):
            ...
            if filter_hits:
                ...
                labels_full[i] = merge.remove_outliers(labels_full[i], all_hits, cells, smallest_track_size=6, aggressive=True, print_counts=False)
                labels_full[i] = merge.renumber_labels(labels_full[i])
...
def predict_event(event_id, hits, cells, train_or_test, truth):
    ...
    labels_helix42 = run_helix_unrolling_predictions(event_id, hits, cells, truth, train_or_test + '_helix42', model_parameters, one_phase_only=True)
    end_time = time.time()
    elapsed_time = end_time - start_time
    print("LIAM: Total model time: {}".format(elapsed_time))
    labels = np.copy(labels_helix42)
...
if __name__ == '__main__':
    ...
    if training_events > 0:
        ...
        print("LIAM: Train path: {}".format(path_to_train))
        print("LIAM: training_skip {} training_events {}".format(training_skip, training_events))
        dataset_scores = []

        for event_id, hits, cells, particles, truth in load_dataset(path_to_train, skip=training_skip, nevents=training_events):
            print("LIAM: event_id: {}".format(event_id))

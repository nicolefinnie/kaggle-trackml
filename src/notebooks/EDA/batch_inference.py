...
def ensemble_predictions(model_names, dbscan=False, first_event=9998, num_events=1, verbose=False):
    ...
    for ev in range(num_events):
        ...
        for i in range(num_models):
            start = time.time()
            pred_raw = gpu_models[i].predict(pred_x[:,:,0:4], batch_size=pred_x.shape[0])
...
def batched_inference_times(model_names, dbscan=False, first_event=9998, num_events=1, batch_size=8192, verbose=False):
